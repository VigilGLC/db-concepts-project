...
public interface PatientRepository extends CrudRepository<Patient, Integer>,
    ...
    @Query(nativeQuery = true,
            value = "select * from PATIENT as patient " +
                    "where patient.REGION = ?1 " +
                    "and " +
                    "?2 = ( select count(*) from " +
                    "(select * from NUCLEIC_ACID_TEST join PATIENT P on P.ID = NUCLEIC_ACID_TEST.PATIENT_ID " +
                    "where P.ID = patient.ID order by DATE limit ?2 )" +
                    "where RESULT= ?3 ) " +
                    "and " +
                    "?4 = ( select count(*) from " +
                    "(select * from INFO_REGISTRATION join PATIENT P2 on INFO_REGISTRATION.PATIENT_ID = P2.ID " +
                    "where P2.ID=patient.ID order by DATE limit ?4)" +
                    "where TEMPERATURE <= ?5 )"
    )
    List<Patient> findPatientsCanDisCharge(Region region, // ?1
